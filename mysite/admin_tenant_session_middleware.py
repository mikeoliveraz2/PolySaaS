        ...
        if request.path.startswith('/admin/') and request.user.is_authenticated:
            ...
            try:
                ...
                if profile and tenant:
                    ...
                    logger.info(f"AdminTenantSessionMiddleware: Set tenant session keys for admin user {request.user.username}: tenant_slug={tenant.slug}, tenant_name={tenant.name}")
                    request.session.save()
                else:
                    logger.warning(f"AdminTenantSessionMiddleware: No UserProfile found for admin user {request.user.username} and tenant_slug={tenant_slug}")
