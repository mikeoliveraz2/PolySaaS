"""
Hosted / container production settings.

Usage:
    export DJANGO_SETTINGS_MODULE=mysite.settings_hosted
...
print("[Django] Loading mysite/settings_hosted.py at", os.path.basename(__file__), flush=True)

from mysite.settings import *  # noqa: F401,F403

print("[Django] Base settings loaded, configuring hosted overrides", flush=True)
...
print("[Django] Liveness middleware inserted at MIDDLEWARE position 0", flush=True)
...
if _allowed:
    ...
    ALLOWED_HOSTS = ["localhost", "127.0.0.1", ".polysaas.online"]
...
# --- Database: DATABASE_URL preferred, else discrete vars ---
...
# --- HTTPS / proxy (platform terminates TLS) ---
SECURE_PROXY_SSL_HEADER = ("HTTP_X_FORWARDED_PROTO", "https")
USE_X_FORWARDED_HOST = True

if not DEBUG:
    # Default False: external proxies often terminate TLS; internal health checks are often HTTP
...
# --- OAuth2 / OIDC: public issuer URL + optional PEM via env ---
...
# --- Logging: explicit stdout for container log streams ---
# Python's StreamHandler defaults to stderr; both streams are captured, but stdout matches
...
# Container: set PYTHONUNBUFFERED=1 (already in Dockerfile.django).
...
print("[Django] mysite/settings_hosted.py fully loaded and ready", flush=True)
