function Invoke-DailyBackup {
    ...
    $sourceFiles = Get-ChildItem -Path $ScriptRoot -Recurse -File -ErrorAction SilentlyContinue | Where-Object {
        $relPath = $_.FullName.Substring($ScriptRoot.Length + 1)
        $dominated = $false
    ...
    foreach ($file in $sourceFiles) {
        $relPath = $file.FullName.Substring($ScriptRoot.Length + 1)
