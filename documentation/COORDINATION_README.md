...
symptom (Slack PolySniffer native browser pane blank) is **not resolved** as of
this entry. Do not mark this BINGO — no screenshot proof of the pane rendering
content, per the screenshot-proof rule in `bingo-freeze.mdc`. Next session should
pick up live debugging in the browser (DevTools Network tab on the actual failing
request) rather than re-deriving the same evidence via test client.

### What was found and fixed (both reproduced with automated before/after evidence)

1. **`dose/middleware/jazzmin_tenant_theme.py`** — `_load_passthrough_endpoints()`
   set `search_path` to the tenant schema alone, with no `,public` fallback, and
   never restored it. Since this middleware runs first on every request, any
   later code needing `public`-schema models (Tenant, User) in that same request
   could silently fail to find rows. Traced back to the 2026-08-08 Odoo/Zoho
   "native pane blank" investigation, whose fix was written but never actually
   applied until today. Fixed: `,public` added back to the `SET search_path`.

2. **`dose/polysniffer/views/sniff_v2_workspace.py` + `dose/templates/polysniffer/sniff_workspace.html`**
   — the workspace page resolves its endpoint correctly via `?schema=<tenant>`,
   but the client-built browser-launch URLs (`/pt/polysniff/<id>/...`, loaded via
   `<object data="...">`, not an iframe) carried no schema hint and depended on
   session state. `AdminTenantSessionMiddleware` (`mysite/admin_tenant_session_middleware.py`)
   resets `session['tenant_slug']` back to the logged-in admin's own home tenant
   on every `/admin/` response, so for any endpoint owned by a *different* tenant
   than the admin's own, the object-tag fetch 404'd and rendered blank (confirmed:
   same endpoint id resolves 200 under the right tenant, 404 under the reset one).
   Fixed: the endpoint's already-resolved schema is now threaded through to the
   client and appended as `?schema=` on the launch URLs, so resolution no longer
   depends on session state a shared admin middleware is entitled to overwrite.
   **Caveat**: the actual browser session in this environment (`Michael.Oliver`,
   home tenant `olient`) already matches Slack's owning tenant (`olient`), so this
   specific mechanism was not necessarily the cause of what was observed live —
   the fix is real and correct in isolation, but did not resolve the user-visible
   symptom on hard reset.

3. **Stale server process** — port 8000 had been served since **2026-08-15** by a
   `waitress-serve` process with no autoreload, while several `manage.py runserver`
   processes from today's session collided on the same port without ever taking
   over. This means most of today's earlier "restart" attempts likely never
   loaded new code at all. Killed the stale process; `runall.ps1`'s own
   supervisor loop (already running in another terminal) respawned Waitress
   fresh, confirmed as the sole listener on port 8000 afterward.

4. **Migration `0060_alter_passthroughendpoint_endpoint_url`** was applied to the
   database in an earlier part of this session but the migration file was never
   committed. Committed now — `showmigrations` confirms `[X]` applied locally;
   verify it applies cleanly with `migrate_all_schemas` on the other machine too.

### What's still open

- Slack PolySniffer native pane (`/admin/polysniffer/sniff/app.slack.com/`)
  remains blank after a hard browser reset, even with both fixes above live and
  the server confirmed freshly restarted. Capture panel keeps polling
  successfully (`GET / 200` entries increment), so the poll/session-tracking path
  works; the actual content pane (`<object data="/pt/polysniff/4/...">`) does not
  render.
- Next step: reproduce with actual browser DevTools open (Network tab) on the
  real failing request to `/pt/polysniff/4/...` while logged in as the real user,
  rather than continuing to infer behavior via `manage.py shell` + Django test
  Client, since the test-client environment does not perfectly mirror the live
  browser session (different user, different session state, as this entry's
  caveat above shows).
- Also worth checking directly: does Slack's own page (loaded inside the
  `<object>` tag) block itself from rendering via `X-Frame-Options` /
  `frame-ancestors` CSP the same way it would in a real `<iframe>`? `<object>`
  tags are subject to the same framing protections as `<iframe>` in modern
  browsers — this was not ruled out and may be the true root cause rather than
  tenant/schema resolution.

### Files changed

- `dose/middleware/jazzmin_tenant_theme.py` (+ `.bak_20260820_searchpath_public_fix`)
- `dose/polysniffer/views/sniff_v2_workspace.py` (+ `.bak_20260820_schema_qualify_urls`)
- `dose/templates/polysniffer/sniff_workspace.html` (+ `.bak_20260820_schema_qualify_urls`)
- `dose/migrations/0060_alter_passthroughendpoint_endpoint_url.py` (new, already applied)
...
**Or:** Test via URL directly: `http://localhost:8000/pt/admin/odoo.prod-polysaas.cloud/web/login`
...
### URLs Configured
- Odoo: `https://odoo.prod-polysaas.cloud`
- Mattermost: `https://mm.prod-polysaas.cloud`
...
2. **Navigate to Odoo passthrough** (e.g. `http://localhost:8000/pt/admin/odoo.prod-polysaas.cloud/`)
