#!/usr/bin/env bash
# Phase 1.3 — Migrate Odoo and Mattermost data from a legacy source to GCP Cloud SQL + GCS.
#
# Prerequisites:
#   - pg_dump / pg_restore installed locally
#   - gsutil installed and authenticated
#   - Legacy source DB credentials in config.env or environment
#
# Usage:
#   bash deploy/gcp/migrate-source-to-gcp.sh [--odoo-only|--mattermost-only]
set -euo pipefail
...
migrate_db() {
  ...
  echo "[migrate] $label: dumping $src_db from source..."
...
if [[ "$MIGRATE_ODOO" -eq 1 ]]; then
  : "${SOURCE_ODOO_DB_HOST:?SOURCE_ODOO_DB_HOST required for Odoo migration}"
  : "${SOURCE_ODOO_DB_USER:?}"
  : "${SOURCE_ODOO_DB_PASSWORD:?}"
  : "${ODOO_DB_PASSWORD:?}"
  migrate_db "odoo" \
    "$SOURCE_ODOO_DB_HOST" "${SOURCE_ODOO_DB_PORT:-5432}" \
    "$SOURCE_ODOO_DB_USER" "$SOURCE_ODOO_DB_PASSWORD" "$SOURCE_ODOO_DB_NAME" \
    "$CLOUDSQL_HOST" "$CLOUDSQL_PORT" \
    "$ODOO_DB_USER" "$ODOO_DB_PASSWORD" "$ODOO_DB_NAME"
fi

if [[ "$MIGRATE_MM" -eq 1 ]]; then
  : "${SOURCE_MM_DB_HOST:?SOURCE_MM_DB_HOST required for Mattermost migration}"
  : "${SOURCE_MM_DB_USER:?}"
  : "${SOURCE_MM_DB_PASSWORD:?}"
  : "${MATTERMOST_DB_PASSWORD:?}"
  migrate_db "mattermost" \
    "$SOURCE_MM_DB_HOST" "${SOURCE_MM_DB_PORT:-5432}" \
    "$SOURCE_MM_DB_USER" "$SOURCE_MM_DB_PASSWORD" "$SOURCE_MM_DB_NAME" \
    "$CLOUDSQL_HOST" "$CLOUDSQL_PORT" \
    "$MATTERMOST_DB_USER" "$MATTERMOST_DB_PASSWORD" "$MATTERMOST_DB_NAME"
fi

echo "[migrate] File migration (optional — run manually if you have source disk exports):"
