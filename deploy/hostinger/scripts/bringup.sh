#!/usr/bin/env bash
# Bring up PolySaaS Hostinger stack (run on vps from repo root or any cwd).
set -euo pipefail
...
VOL_NAME=$(docker volume ls --format '{{.Name}}' | grep -E 'host_letsencrypt$' | head -n1 || true)
if [[ -z "${VOL_NAME}" ]]; then
...
echo ""
echo "Next:"
echo "  1. Confirm DNS A records for app/mm/odoo → VPS IP"
