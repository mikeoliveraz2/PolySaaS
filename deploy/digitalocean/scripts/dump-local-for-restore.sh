# Dump local DBs for DigitalOcean restore (run on laptop / office machine).
# Requires: docker (local compose) or pg_dump against localhost ports.
set -euo pipefail

OUT_DIR="${1:-deploy/digitalocean/restore}"
mkdir -p "$OUT_DIR"
...
echo ""
echo "Copy to droplet:"
echo "  scp $OUT_DIR/*.sql.gz root@<FLOATING_IP>:/opt/polysaas/deploy/digitalocean/restore/"
echo "Then on droplet: bash deploy/digitalocean/scripts/migrate-data.sh"
