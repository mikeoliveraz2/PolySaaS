#!/usr/bin/env bash
# Restore dumps into running DO stack volumes (run on droplet).
# Place dumps first:
#   /opt/polysaas/deploy/digitalocean/restore/core.sql.gz
#   /opt/polysaas/deploy/digitalocean/restore/odoo.sql.gz   (optional)
#   /opt/polysaas/deploy/digitalocean/restore/mattermost.sql.gz (optional)
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
DO_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
ENV_FILE="$DO_DIR/.env"
RESTORE_DIR="$DO_DIR/restore"
