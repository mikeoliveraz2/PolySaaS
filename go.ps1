# go.ps1 - PolySaaS Launcher: Pull -> App check -> (if OK) Commit/Push -> Services -> runserver -> (on exit) pip freeze
# Daily zip backup runs in EOD.ps1, not here.
#
...
. (Join-Path $goDir "Go-Worktree.ps1")
. (Join-Path $goDir "Go-MorningSync.ps1")
. (Join-Path $goDir "Go-Services.ps1")
. (Join-Path $goDir "Go-SessionEnd.ps1")

# ── Pull first (main checkout + worktree — always both) ────────────────

Write-Host ""
Write-Host "── Pull ───────────────────────────────────────────────" -ForegroundColor Cyan
$pullOk = Invoke-PolySaaSGitPullAll -ScriptRoot $scriptDir
if (-not $pullOk) {
    Write-Host "  Pull failed in main checkout or worktree - fix before continuing" -ForegroundColor Red
}
Write-Host ""
...
# ── App load check: only if this passes do we commit/push ──────────────
...
if ($appLoadOk) {
    if (Test-PolySaaSGitHasUnmergedFiles -RepoRoot $scriptDir) {
        Write-Host '  App loads OK - git sync skipped because merge conflicts are unresolved' -ForegroundColor Yellow
    } else {
        Write-Host '  App loads OK - will commit/push now' -ForegroundColor Green
    }
} else {
    Write-Host "  App failed to load - skipping commit/push" -ForegroundColor Yellow
...
# Django runs from the wip worktree when present (.worktree-path or defaults).
# Main checkout stays on branch main for pull/EOD; WIP commits happen on branch wip.
if (-not $worktreeRoot) {
    $worktreeRoot = Get-PolySaaSWorktreeRoot -RepoRoot $scriptDir
}
$worktreeManage = if ($worktreeRoot) { Join-Path $worktreeRoot "manage.py" } else { "" }

if ($worktreeRoot -and (Test-Path $worktreeManage)) {
