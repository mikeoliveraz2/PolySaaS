...
# BINGO: Mattermost Browse search_path — 2026-09-19 — commit b8b827cd — see documentation/BINGO_MM_BROWSE_SEARCHPATH_TENANT_SCHEMA_2026-09-19.md
"""
...
class PassthroughAuthMiddleware(MiddlewareMixin):
    ...
    def _inject_app_token(self, request):
        """
        Look up the app slug from the path, find TenantApp in the *tenant*
        schema, and inject the app-specific auth token if one is stored.

        FIX 2026-09-19: Never SET search_path to public here. That left
        ExternalPassthroughMiddleware querying PassThroughEndpoint in public
        after /pt/admin/mattermost/ — "No enabled PassThroughEndpoint with
        slug mattermost" while the row lived in the tenant schema. TenantApp
        is tenant-scoped; query it there and keep search_path on the tenant.
        """
        ...
        try:
            ...
            ta = TenantApp.objects.filter(app_name=trigger, status='active').first()
            extra_config = (ta.extra_config if ta else None) or {}
