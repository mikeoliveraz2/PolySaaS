...
class HubspotPassthroughHandler(PassthroughHandlerBase):
    ...
    def process_html_response(self, html_str, request, endpoint_url=None, **context):
        ...
        prefix = proxy_prefix_for_trigger_endpoint(endpoint) if endpoint else self._proxy_prefix_from_url(endpoint_url)
        base = self._base_from_url(endpoint_url or getattr(endpoint, 'endpoint_url', ''))
        return self._rewrite_html(html_str, prefix, base)
