...
class NextcloudPassthroughHandler:
    ...
    def process_html_response(self, html_str, response, endpoint_url=None, *args, **kwargs):
        ...
        # Inject shim + _oc_webroot before </head> so Vue uses correct paths
        shim = self._build_early_shim(proxy_prefix, base)
        if "</head>" in html_str:
            html_str = html_str.replace("</head>", shim + "</head>", 1)
    ...
    def _rewrite_nc_paths(self, html: str, proxy_prefix: str, base: str) -> str:
        ...
        """
        # Patterns that should be proxied (navigation + APIs + assets)
        _NC_PROXY_PREFIXES = (
            "/index.php/",
            "/core/",
            "/apps/",
