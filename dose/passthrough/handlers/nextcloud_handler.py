...
class NextcloudPassthroughHandler:
    ...
    def _get_tenantapp_extra_config(self, request=None) -> dict:
        """Return TenantApp.extra_config for this tenant's Nextcloud app."""
        try:
            ...
            if ta and ta.extra_config:
                extra = ta.extra_config
                if isinstance(extra, str):
                    return json.loads(extra)
                return dict(extra)
        except Exception as exc:
            logger.warning("[NC-HANDLER] extra_config lookup failed: %s", exc)
        return {}

    def _get_nc_credentials(self, request) -> tuple:
        """(login, password) from TenantApp.extra_config; empty strings if missing."""
        extra = self._get_tenantapp_extra_config(request) or {}
        login = (
            extra.get('nc_login')
            or extra.get('nextcloud_login')
            or extra.get('nc_userid')
            or ''
        )
        password = (
            extra.get('nc_password')
            or extra.get('nextcloud_password')
            or ''
        )
        return (str(login).strip(), str(password))

    def _server_side_nc_login(self, base_url: str, username: str, password: str):
        """
        Form-login to Nextcloud; return (ok, set_cookie_raw_list).
        Mirrors browser login: GET /login → requesttoken → POST /login.
        """
        ...
        # Prefer 127.0.0.1 for host→docker reliability while keeping Cookie Domain unset.
        fetch_base = base.replace('localhost', '127.0.0.1')
        try:
            s = _rq.Session()
            r = s.get(f"{fetch_base}/login", timeout=20)
            if r.status_code != 200:
                logger.warning("[NC-SSO] GET /login HTTP %s", r.status_code)
                return False, []
            m = re.search(r'data-requesttoken="([^"]+)"', r.text, re.I)
            if not m:
                m = re.search(r'name="requesttoken"\s+value="([^"]+)"', r.text, re.I)
            tok = m.group(1) if m else ''
            if not tok:
                logger.warning("[NC-SSO] no requesttoken on login page")
                return False, []
            ...
            # Success: redirect away from login (dashboard/files) or Set-Cookie nc_username
            cookie_names = {c.name for c in s.cookies}
            logged_in = (
                'nc_username' in cookie_names
                or 'nc_session_id' in cookie_names
                or ('/login' not in loc and r2.status_code in (301, 302, 303, 307) and loc)
            )
            if not logged_in:
                # Confirm via OCS with the session jar
                probe = s.get(
                    f"{fetch_base}/ocs/v2.php/cloud/user",
                    headers={'OCS-APIRequest': 'true', 'Accept': 'application/json'},
                    timeout=10,
                )
                logged_in = probe.status_code == 200 and 'not logged in' not in (probe.text or '').lower()
            if not logged_in:
                logger.warning(
                    "[NC-SSO] login failed for user=%s POST=%s loc=%s",
                    username, r2.status_code, loc[:80],
                )
                return False, []
            raw_cookies = []
            for c in s.cookies:
                raw_cookies.append(f"{c.name}={c.value}; Path=/; HttpOnly; SameSite=Lax")
            print(f"[NC-SSO] server login OK user={username} cookies={len(raw_cookies)}")
            return True, raw_cookies
        except Exception as exc:
            logger.warning("[NC-SSO] exception: %s", exc)
            return False, []

    def _sso_redirect_with_cookies(self, proxy_prefix: str, set_cookie_raw_list):
        """Redirect into the Files app with upstream session cookies on the browser."""
        target = (proxy_prefix.rstrip('/') + '/apps/files/')
        resp = HttpResponseRedirect(target)
        self._apply_set_cookies(resp, set_cookie_raw_list)
        return resp

    def try_nextcloud_sso(self, request, endpoint, proxy_prefix: str):
        """
        If browser has no NC session, authenticate with provisioned credentials and
        return a redirect response with Set-Cookie. Returns None if SSO not possible.
        """
        login, password = self._get_nc_credentials(request)
        if not login or not password:
            print("[NC-SSO] no credentials in TenantApp.extra_config")
            return None
        base = (getattr(endpoint, 'endpoint_url', None) or '').rstrip('/')
        if not base:
            return None
        ok, cookies = self._server_side_nc_login(base, login, password)
        if not ok:
            return None
        return self._sso_redirect_with_cookies(proxy_prefix, cookies)

    def passthrough_early_shell_paths(self):
        """
        Middleware only calls try_root_display_shell_response for these upstream
        subpaths (plus children). Needed so /apps/files/ and /login hit SSO before
        the forwarder (which only auto-invokes try_root for '/').
        """
        return (
            '/',
            '/login',
    ...
    def try_root_display_shell_response(self, request, endpoint, url_trigger_segment):
        ...
        # Login path: try server SSO first (provisioned creds), else fall through to NC login HTML.
        if upstream_subpath.rstrip("/") in ("/login", "/index.php/login"):
            if endpoint is not None:
                sso = self.try_nextcloud_sso(request, endpoint, proxy_prefix)
                if sso is not None:
                    return sso
            return None
        ...
        if endpoint is not None:
            ...
            if _nc_shell_path_needs_nc_session(fetch_path):
                ...
                if not NextcloudPassthroughHandler._nc_upstream_ocs_user_ok(_fb, request):
                    print("[NC_OCS] user probe failed — attempting server SSO")
                    sso = self.try_nextcloud_sso(request, endpoint, proxy_prefix)
                    if sso is not None:
                        return sso
                    print(
                        "[NC-HANDLER] SSO unavailable — redirect to login "
...
function _ncReqLooksLikeLogin(reqUrl){{
    if(!reqUrl||typeof reqUrl!=='string')return false;
    if(reqUrl.indexOf('/login')!==-1||reqUrl.indexOf('index.php/login')!==-1)return true;
    try{{
        var p=new URL(reqUrl,O).pathname.toLowerCase();
        return p.endsWith('/login')||p.indexOf('/index.php/login')!==-1;
    }}catch(e){{return false;}}
}}

function _ncResolveToSameOriginNav(u){{
    if(!u||typeof u!=='string')return null;
    u=u.trim();
    if(!u)return null;
    var out;
    if(u.indexOf('http://')===0||u.indexOf('https://')===0){{out=u;}}
    else{{
        var rel=u.charAt(0)==='/'?u:'/'+u;
        rel=_toProxy(rel);
        try{{out=new URL(rel,O).href;}}catch(e){{return null;}}
    }}
    try{{
        var x=new URL(out);
        if(x.origin!==new URL(O).origin)return null;
        if(_ncPathLooksLikeLogin(x.pathname))return null;
        return out;
    }}catch(e){{return null;}}
}}

function _ncMaybeNavigateFromNcLoginResponse(text){{
    if(!text)return;
    var t=text.trim();
    ...
    try{{
        ...
        if(j&&typeof j==='object'){{
            if(typeof j.url==='string')cands.push(j.url);
            if(typeof j.redirectUrl==='string')cands.push(j.redirectUrl);
            if(typeof j.redirectURL==='string')cands.push(j.redirectURL);
            if(typeof j.location==='string')cands.push(j.location);
            if(j.data&&typeof j.data==='object'&&typeof j.data.url==='string')cands.push(j.data.url);
            if(j.ocs&&j.ocs.data&&typeof j.ocs.data.url==='string')cands.push(j.ocs.data.url);
        }}
        for(var i=0;i<cands.length;i++){{
            var nav=_ncResolveToSameOriginNav(cands[i]);
            if(nav){{window.location.assign(nav);return;}}
        }}
    }}catch(e){{}}
}}
...
window.fetch=function(input,init){{
    ...
    return _f.call(this,input,init).then(function(resp){{
        try{{
            if(!resp||_method.toUpperCase()!=='POST'||!_ncReqLooksLikeLogin(_reqUrl))return resp;
            var finalU=resp.url||'';
            if(!finalU||finalU.indexOf(O)!==0)return resp;
            var pu=new URL(finalU);
            if(!_ncPathLooksLikeLogin(pu.pathname)){{
                if(resp.redirected||resp.ok)window.location.assign(finalU);
                return resp;
            }}
            if(resp.ok&&resp.clone){{
                return resp.clone().text().then(function(txt){{
                    _ncMaybeNavigateFromNcLoginResponse(txt);
                    return resp;
...
XMLHttpRequest.prototype.send=function(){{
    ...
    var onDone=function(){{
        try{{
            ...
            if(!_ncReqLooksLikeLogin(self._polyNcUrl||''))return;
            ...
            if(self.responseText){{
                ...
                if(ct.indexOf('json')!==-1||self.responseText.trim().charAt(0)==='{{')
                    _ncMaybeNavigateFromNcLoginResponse(self.responseText);
            }}
        }}catch(e){{}}
    }};
    self.addEventListener('load',onDone,{{once:true}});
