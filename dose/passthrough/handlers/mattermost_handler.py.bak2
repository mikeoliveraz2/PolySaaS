...
// Route URLs:
//   API/dynamic paths  → through PolySaaS proxy (server-side auth injection)
//   Non-PolySaaS paths → direct to upstream Mattermost (static chunks, images, etc.)
//   PolySaaS paths     → untouched
function toProxy(s) {{
  if (typeof s !== 'string') return s;
  if (!s || s.startsWith('data:') || s.startsWith('blob:')) return s;
  // Already absolute upstream URL — leave alone
  ...
  s = stripMmSubpath(s);
  if (isMMApiPath(s)) return PROXY + s;
  // All other MM root-relative paths (static chunks, fonts, images) → direct to upstream.
  // This is the key fix: webpack loads chunks as /static/NNNN.hash.js which would
  // otherwise resolve against localhost:8000 instead of localhost:8065.
  return B + s;
}}
var _f = window.fetch;
...
// WebSocket — direct to Mattermost upstream.
// CRITICAL: use the LIVE session token from localStorage, NOT the static PAT (T).
// The PAT may be expired/revoked; after a fresh login the SPA updates localStorage['MMAUTHTOKEN']
// with the new session token. Reading from localStorage here ensures every reconnect uses the
// most current token, breaking the "invalid session immediately after login" loop.
var _WS = WebSocket;
window.WebSocket = function(url, protocols) {{
  if (typeof url === 'string') {{
    try {{
      ...
      console.log('=== WS TOKEN DEBUG ===');
      console.log('T (PAT at render):', T ? T.substring(0,8)+'...' : 'none');
      console.log('S (server session at render):', S ? S.substring(0,8)+'...' : 'none');
      try {{
        var _lt = localStorage.getItem('MMAUTHTOKEN');
        console.log('localStorage MMAUTHTOKEN:', _lt ? _lt.substring(0,8)+'...' : 'none');
      }} catch(e) {{ console.log('localStorage read error:', e); }}
      console.log('Chosen WS token:', wsToken ? wsToken.substring(0,8)+'...' : 'NONE — WS will fail');
      console.log('======================');
