...
class MattermostPassthroughHandler:
    ...
    def _inject_client_shim(self, html, base_origin, request, proxy_prefix):
        """Strong shim: token, __webpack_public_path__, createElement patch. First in <head>."""
        token = ""
        try:
...
(function() {{
    ...
    if (MMAUTHTOKEN) {{
        try {{
            ...
            console.log('[PolySaaS STRONG] MMAUTHTOKEN injected');
        }} catch (e) {{
            console.warn('[PolySaaS STRONG] Token injection failed:', e);
        }}
    }}
    ...
    document.createElement = function(tag) {{
        ...
        if (tag.toLowerCase() === 'script' || tag.toLowerCase() === 'link') {{
            var prop = tag.toLowerCase() === 'script' ? 'src' : 'href';
    ...
    console.log('[PolySaaS STRONG] Shim with dynamic patching loaded');
}})();
</script>
"""
