...
class SlackPassthroughHandler(PassthroughHandlerBase):
    ...
    def native_session_cookies(self, request=None, *, tenant_schema="", endpoint_host=""):
        """Playwright cookies for a prior Slack session, applied before first paint.

        Black box: we never fill Slack's login form. We only re-present cookies
        Slack itself issued on an earlier Native run. Stored next to the profile
        (not in TenantApp — Slack is not a provisioned APP_CHOICES entry yet).
        """
        path = self._native_cookie_store_path(tenant_schema, endpoint_host)
        ...
        if not isinstance(data, list):
            return []
        return [c for c in data if isinstance(c, dict) and c.get("name") and c.get("value") is not None]

    def remember_native_session_cookies(
        self, cookies, request=None, *, tenant_schema="", endpoint_host=""
    ):
        """Keep Slack-issued cookies for the next Native start's first paint."""
        path = self._native_cookie_store_path(tenant_schema, endpoint_host)
        if not path:
            return
        keep = []
    ...
    @staticmethod
    def _native_cookie_store_path(tenant_schema: str, endpoint_host: str):
        if not tenant_schema or not endpoint_host:
            return None
        import re
        ...
        safe = lambda v: re.sub(r"[^a-zA-Z0-9_.-]+", "_", v).strip("._") or "capture"
        return (
    ...
    def stray_root_paths(self):
        ...
        When the credential iframe times out, Slack's client falls back to
        `window.location = '/auth?...'`, which lands on our root instead of the
        proxy prefix. No client shim can catch it (window.location is
        unforgeable), so the relay middleware redirects it back onto the prefix.
        """
        return _SLACK_HOST_ROOT_PREFIXES
