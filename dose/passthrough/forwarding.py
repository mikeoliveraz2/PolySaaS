...
# BINGO: PolySniffer 2.0 har_capture passthrough logging — 2026-06-24
...
def _extract_upstream_root_classes(html):
    """Collect class names from upstream <html> and <body> (Odoo o_dark lives here)."""
    import re
    found = []
    for pattern in (
        r'<html[^>]*class=["\']([^"\']*)["\']',
...
def _wrap_in_admin_template(request, response, trigger, endpoint, handler=None):
    ...
    upstream_root_classes = _extract_upstream_root_classes(raw_html)
        ...
        try:
            scope_extra_classes = (
                handler.passthrough_embed_scope_classes(
                    request, raw_html, upstream_root_classes,
                )
    ...
    if handler is None:
        ...
        handler = resolve_handler_for_pt_admin_trigger(trigger or '')
    if handler and hasattr(handler, 'passthrough_embed_template_context'):
        template_ctx.update(handler.passthrough_embed_template_context(trigger, request))
    try:
...
def forward_request_standardized(request, endpoint_url, handler=None, endpoint=None, trigger=None):
    ...
    try:
        ...
        try:
            ...
            _cap = getattr(request, '_polysniffer_capture', None) or get_active_capture_session(
                request, getattr(request, '_polysniffer_endpoint_id', None)
            )
            if _cap:
                _sniff_mode = getattr(request, '_polysniffer_sniff_mode', 'passthrough') or 'passthrough'
                log_requests_response(
                    request,
                    resp,
                    capture_source='passthrough',
                    target_url=target_url,
                    upstream_path=upstream_path,
                    endpoint_name=app_name,
                    service=app_name,
                    sniff_mode=_sniff_mode,
                    duration_ms=0,
                )
                print(f"POLY SNIFFER 2.0 — Captured {request.method} {upstream_path} ({_sniff_mode})")
        except Exception as ps_exc:
