"""
CreateGitHubIssue — create an issue in a configured GitHub repository.
"""
...
class CreateGitHubIssue(AtomicServiceBase):

    @staticmethod
    def get_parameters(parameters, key='CreateGitHubIssue'):
    ...
    def execute_and_save(request, instruction_row):
        ...
        if not token:
            return service_result('CreateGitHubIssue', status='error', error='GITHUB_TOKEN not configured')
        ...
        try:
            ...
            result = service_result('CreateGitHubIssue', status='error', error=str(exc))
