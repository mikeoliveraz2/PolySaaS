"""
GenerateImageAndExportService — generate an image via xAI (or mock) and optional export URL.
"""
...
class GenerateImageAndExportService(AtomicServiceBase):

    @staticmethod
    def get_parameters(parameters, key='GenerateImageAndExportService'):
    ...
    def _export_url(export_url, image_path_or_url, cfg):
        ...
        payload = cfg.get('export_body') or {
            ...
            'source': 'GenerateImageAndExportService',
        }
        try:
    ...
    def execute_and_save(request, instruction_row):
        ...
        image_data, provider = GenerateImageAndExportService._generate_image(prompt, api_key)
        local_path = None
        if image_data:
            if isinstance(image_data, bytes):
                fname = cfg.get('filename') or f"orch_{getattr(instruction_row, 'id', 'new')}.png"
                local_path = GenerateImageAndExportService._save_local(image_data, fname)
            else:
                local_path = image_data
        else:
            provider = 'mock'
            fname = cfg.get('filename') or f"orch_{getattr(instruction_row, 'id', 'new')}_placeholder.txt"
            local_path = GenerateImageAndExportService._save_local(b'', fname)
            Path(local_path).write_text(f'[mock image] prompt={prompt}\n', encoding='utf-8')

        export_url = cfg.get('export_url') or cfg.get('nextcloud_url')
        export_result = GenerateImageAndExportService._export_url(export_url, local_path, cfg)
