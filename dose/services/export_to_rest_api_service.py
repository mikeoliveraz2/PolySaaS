"""
ExportToRESTAPI — POST/PUT/PATCH data to any external REST endpoint.
"""
...
class ExportToRESTAPI(AtomicServiceBase):

    @staticmethod
    def get_parameters(parameters, key='ExportToRESTAPI'):
    ...
    def execute_and_save(request, instruction_row):
        ...
        if not url:
            return service_result('ExportToRESTAPI', status='error', error='missing_url')
        ...
        try:
            ...
            result = service_result('ExportToRESTAPI', status='error', url=url, error=str(exc))
