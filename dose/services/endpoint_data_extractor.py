"""
EndpointDataExtractorService - Atomic service that intercepts bundled app
...
class EndpointDataExtractorService(AtomicServiceBase):

    @staticmethod
    def get_parameters(parameters):
        key = 'EndpointDataExtractorService'
    ...
    def execute_and_save(request, instruction_row):
        ...
        normalized, topic, app_name, entity_name, action = \
            EndpointDataExtractorService._try_mapping_engine(
        ...
        publish_result = EndpointDataExtractorService._publish_to_pubsub(topic, message)

        callback_data = {
            "service_name": "EndpointDataExtractorService",
