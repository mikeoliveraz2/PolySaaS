...
class OdooInvoiceNotifier(AtomicServiceBase):
    ...
    def _load_mm_config(request):
        config = {
            'mm_url': 'https://mm.prod-polysaas.cloud',
            'mm_token': '',
        ...
        try:
            ...
            # TenantApp is tenant-owned data; look it up in the tenant's own schema.
            ta = None
            if tenant:
                with tenant_schema_search_path(tenant):
                    for _app_name in ('mattermost', 'odoo'):
                        candidate = TenantApp.objects.filter(
                            app_name=_app_name, tenant=tenant,
                        ).first()
