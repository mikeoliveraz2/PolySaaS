"""
NotifyAIPeers — broadcast orchestration context to Mattermost AI peers.
"""
...
class NotifyAIPeers(AtomicServiceBase):

    @staticmethod
    def get_parameters(parameters, key='NotifyAIPeers'):
    ...
    def execute_and_save(request, instruction_row):
        ...
        if not channel_id:
            return service_result('NotifyAIPeers', status='error', error='missing_channel_id')
