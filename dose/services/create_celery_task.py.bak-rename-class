"""
CreateCeleryTaskService — queue a Celery background task by name.
"""
...
class CreateCeleryTaskService(AtomicServiceBase):

    @staticmethod
    def get_parameters(parameters, key='CreateCeleryTaskService'):
    ...
    def execute_and_save(request, instruction_row):
        ...
        if not task_name:
            return service_result('CreateCeleryTaskService', status='error', error='missing_task_name')
