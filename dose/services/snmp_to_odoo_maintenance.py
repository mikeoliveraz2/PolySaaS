"""
SnmpToOdooMaintenance — Type 2 mailbox capture + Type 1 Odoo Maintenance feed.

POST /dose/webhook/snmp/<tenant_slug>/
  → enroll WebhookMailbox (raw SNMP history)
  → upsert maintenance.equipment (by device_mac → serial_no)
  → on anomaly: create maintenance.request

Demo: run Week 1 then Week 2 feeds immediately (different data, not calendar wait).
"""
...
class SnmpToOdooMaintenance(AtomicServiceBase):
    """SNMP JSON → mailbox + Odoo maintenance.equipment / maintenance.request."""
    ...
    def execute_and_save(request, instruction_row):
        ...
        temp_critical = int(cfg.get("temp_critical_c", DEFAULT_TEMP_CRITICAL))
        create_requests = bool(cfg.get("create_requests", True))
        ...
        try:
            odoo = SnmpToOdooMaintenance._feed_odoo(
                request,
                payload,
            ...
            logger.error("[SNMP→Odoo] RPC failed: %s", exc)
            return service_result(
                "SnmpToOdooMaintenance",
                status="error",
                error=str(exc),
            ...
            logger.exception("[SNMP→Odoo] unexpected failure")
            return service_result(
                "SnmpToOdooMaintenance",
                status="error",
                error=str(exc),
        ...
        return service_result(
            ...
            mailbox=mailbox,
            odoo=odoo,
            anomaly=bool(odoo.get("request_id")),
        )
