"""
WriteToBigQueryService — insert orchestration rows into BigQuery (ADC or service account).
"""
...
class WriteToBigQueryService(AtomicServiceBase):

    @staticmethod
    def get_parameters(parameters, key='WriteToBigQueryService'):
    ...
    def execute_and_save(request, instruction_row):
        ...
        try:
            ...
            result = service_result('WriteToBigQueryService', status='error', error=str(exc), row_preview=row)
