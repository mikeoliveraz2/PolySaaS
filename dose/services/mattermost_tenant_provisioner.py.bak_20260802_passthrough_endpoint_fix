...
def sync_mattermost_users_shared_team_landing(
    ...
    *,
    ...
    TenantApp is tenant-owned data and lives only in each tenant's own schema
    (never in public), so this scans schema-by-schema rather than a single query.
    """
    from dose.models import Tenant, TenantApp
    from dose.tenant_app_lookup import tenant_schema_search_path_by_name
    ...
    for schema in schemas:
        with tenant_schema_search_path_by_name(schema):
            rows = list(TenantApp.objects.filter(app_name__icontains='mattermost'))
        for ta in rows:
...
def _store_credentials(
    ...
    *,
    ...
    try:
        ...
        with tenant_schema_search_path_by_name(tenant_schema):
...
def ensure_dual_mattermost_teams(
    ...
    try:
        ...
        # TenantApp is tenant-owned data; it lives only in the tenant's own schema.
        with connection.cursor() as cursor:
        ...
        # Re-assert tenant schema before saving — the HTTP calls above don't touch the
        # DB, but this must not drift back to public (TenantApp is tenant-owned data).
        with connection.cursor() as cursor:
            cursor.execute(f'SET search_path TO "{tenant.schema_name}", public')
