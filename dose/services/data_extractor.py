...
API calls, extracts entity data, normalizes it, and publishes to MQ
for downstream consumption by other atomic services.
...
Flow:
  ...
  4. Publishes to RabbitMQ (local primary) with topic
     {REQ|RES}.{action_path}.{username} — see dose.utils.mq_topic
     (defaults to REQ for request-path extractors)
  5. Downstream atomic service picks up via MQQueueMonitor → MQRequestController

GCP Pub/Sub remains a fallback when an active google_pubsub MQConfig exists
and no rabbitmq config is active.
"""
import json
import logging
import datetime
...
from dose.utils.mq_topic import topic_from_request
...
class EndpointDataExtractor(AtomicServiceBase):
    ...
    def execute_and_save(request, instruction_row):
        ...
        normalized, _mapped_topic, app_name, entity_name, action = \
        ...
        # Locked convention: {REQ|RES}.{action_path}.{username}
        action_path = (
            getattr(instruction_row, "requestpath", None)
            or request_path
        )
        direction = getattr(instruction_row, "direction", None) or "REQ"
        topic = topic_from_request(
            request, action_path=action_path, direction=direction
        )
        ...
        publish_result = EndpointDataExtractor._publish_to_mq(topic, message)
        ...
        try:
            if instruction_row and getattr(instruction_row, 'save_callbackdata', False):
                ...
                if not tenant or not ensure_tenant_search_path(tenant, 'data_extractor_callback'):
                    logger.warning('[EndpointDataExtractor] CallBackData skipped — no tenant context')
                else:
    ...
    @staticmethod
    def _publish_to_mq(topic, message):
        """
        Publish a message to the configured MQ provider.
        Local primary: RabbitMQ. GCP Pub/Sub remains a fallback.
        """
        try:
            from dose.models.mq_config import MQConfig

            # --- RabbitMQ first (local Odoo path / runall) ---
            ...
            if rabbit_configs.exists():
                ...
                # No MQConfig row yet — still try localhost defaults from runall
                class _LocalRabbit:
                ...
                try:
                    from dose.mq.adapters.rabbitmq_adapter import RabbitMQAdapter
                    adapter = RabbitMQAdapter(_LocalRabbit())
                    if adapter.connect():
                        result = adapter.publish(message, routing_key=topic)
                        adapter.close()
                        logger.info(
                            f"[EndpointDataExtractor] Published to local RabbitMQ {topic}: {result}"
                        )
                        return result
                    adapter.close()
                except Exception as local_exc:
                    logger.warning(
                        f"[EndpointDataExtractor] Local RabbitMQ publish failed: {local_exc}"
                    )

            # --- GCP Pub/Sub via ADC (optional map) ---
    ...
    def _publish_to_pubsub(topic, message):
        """Back-compat alias — prefer `_publish_to_mq`."""
