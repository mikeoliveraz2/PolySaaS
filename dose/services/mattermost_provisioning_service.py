"""
MattermostProvisioningService — Atomic Service for provisioning a Mattermost
...
(matchingKey = 'MattermostProvisioningService'). No hardcoded config.

Triggered via an Instruction matching the subscription event. The Instruction's
executescript field should be set to 'MattermostProvisioningService'.
...
class MattermostProvisioningService(AtomicServiceBase):

    @staticmethod
    def get_parameters(parameters, key='MattermostProvisioningService'):
        if isinstance(parameters, list):
            return [
                p for p in parameters
                if (hasattr(p, 'matchingKey') and p.matchingKey == key)
                or (isinstance(p, dict) and p.get('matchingKey') == key)
            ]
    ...
    def execute_and_save(request, instruction_row):
        ...
        config = MattermostProvisioningService._load_config(params)
        if not config:
            msg = 'Missing Parameters for MattermostProvisioningService'
            logger.error('[MM-PROVISION] %s', msg)
            return {'success': False, 'error': msg}
        ...
        try:
            team_id = MattermostProvisioningService._create_team(
                mm_url, headers, tenant, team_type, result,
            )
            user_id = MattermostProvisioningService._create_user(
                mm_url, headers, user, result, request=request,
            )
            if team_id and user_id:
                MattermostProvisioningService._add_user_to_team(
