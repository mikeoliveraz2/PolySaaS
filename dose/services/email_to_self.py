"""
EmailToSelf — send a formatted email to the authenticated user.
"""
...
class EmailToSelf(AtomicServiceBase):

    @staticmethod
    def get_parameters(parameters, key='EmailToSelf'):
    ...
    def execute_and_save(request, instruction_row):
        ...
        if not user or not getattr(user, 'is_authenticated', False):
            return service_result('EmailToSelf', status='error', error='not_authenticated')

        recipient = getattr(user, 'email', '') or ''
        if not recipient:
            return service_result('EmailToSelf', status='error', error='no_user_email')
        ...
        try:
            ...
            result = service_result('EmailToSelf', status='error', error=str(exc))
