"""
Seed Mattermost message → Odoo contact creation orchestration Instructions.

Creates tenant-scoped Instructions so the orchestration hook fires when a
Mattermost message with contact format is received, then wires it to the
OdooCreatePartner atomic service.

Parallel to seed_slack_contact_orchestration.py but for Mattermost source.

Usage:
    python manage.py seed_mattermost_orchestration             # default: polysaasonline
    python manage.py seed_mattermost_orchestration <slug>      # specific tenant
    python manage.py seed_mattermost_orchestration --dry-run   # preview only
"""
...
class Command(BaseCommand):
    help = "Seed Mattermost → Odoo contact creation Instructions for a tenant."

    def add_arguments(self, parser):
        parser.add_argument(
            "tenant_slug",
            nargs="?",
            default="polysaasonline",
            help="Tenant slug (default: polysaasonline)",
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
        ...
        parser.add_argument(
            "--delete-existing",
            action="store_true",
            help="Delete existing mattermost.message.contact Instructions before re-seeding",
        )
    ...
    def handle(self, *args, **options):
        ...
        # Mattermost Outgoing Webhook — message with contact format
        instruction_spec = {
