...
def debug_dashboard(request):
    """
    Debug dashboard - shows captured traffic logs.

    Query params:
      source=passthrough — only rows logged by Django passthrough forwarder (full cookies, client_path).
      source=browser — extension / silent-capture rows (capture_source=browser_extension).
      (omit source) — all rows, including legacy rows with empty capture_source.
      endpoint=... — filter by endpoint_name (icontains)
    ...
    qs = TrafficLog.objects.all().order_by('-captured_at')
    source = (request.GET.get('source') or '').strip().lower()
    if source == 'passthrough':
        qs = qs.filter(capture_source=TrafficLog.CAPTURE_PASSTHROUGH)
    elif source in ('browser', 'browser_extension', 'ext'):
        qs = qs.filter(capture_source=TrafficLog.CAPTURE_BROWSER_EXT)

    ep = (request.GET.get('endpoint') or '').strip()
    if ep:
        qs = qs.filter(endpoint_name__icontains=ep)
    ...
    endpoints = (
        TrafficLog.objects.exclude(endpoint_name='')
        .values_list('endpoint_name', flat=True)
        .distinct()
        .order_by('endpoint_name')[:50]
    )
...
def export_har(request, log_id=None):
    ...
    if log_id:
        ...
        logs = TrafficLog.objects.all().order_by('-captured_at')[:100]
    ...
    for log in logs:
        entry = {
            "startedDateTime": log.captured_at.isoformat() if log.captured_at else "",
            "time": 0,
            "request": {
                "method": log.method,
                "url": log.url,
...
def capture_traffic(request):
    ...
    try:
        ...
        raw_url = str(capture_data.get('url', ''))[:500] or 'http://capture/unknown'
        raw_path = raw_url.split('?')[0].split('#')[0][:500] or '/'
        hdrs = capture_data.get('headers', {})
        if not isinstance(hdrs, dict):
            hdrs = {}
        req_body = capture_data.get('body', '')
        if not isinstance(req_body, str):
            req_body = json.dumps(req_body)

        TrafficLog.objects.create(
            method=str(capture_data.get('method', 'GET'))[:10],
            url=raw_url,
            path=raw_path,
            headers=hdrs,
            ...
            query_params={},
            body=req_body,
            status_code=int(capture_data.get('status', 0) or 0),
            response_headers=capture_data.get('response_headers', {}),
            response_body=json.dumps(capture_data.get('data', {})) if capture_data.get('data') else '',
            endpoint_name=str(capture_data.get('endpoint_name', 'generic_capture'))[:200],
            user=request.user,
            captured_at=timezone.now(),
            duration_ms=float(capture_data.get('duration_ms', 0) or 0),
            capture_source=str(capture_data.get('capture_source', ''))[:32],
            client_path='',
        )
        ...
        for capture in captures:
            try:
                TrafficLog.objects.create(
                    ...
                    user=request.user,
                    captured_at=timezone.now(),
                    capture_source=TrafficLog.CAPTURE_BROWSER_EXT,
                )
                log_count += 1
            except Exception:
                pass
