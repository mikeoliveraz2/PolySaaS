...
def _harvest_via_passthrough(request, endpoint_id: int) -> bool:
    ...
    try:
        ...
        resp = dispatch_polysniff_passthrough(request, endpoint_id, 'home/')
        ...
        logger.warning('[SYNC-SESSION] harvest_via_passthrough failed eid=%s: %s', endpoint_id, exc)
...
def sync_hubspot_session(request, endpoint_id: int):
...

    {
        ...
        "action": "store" | "probe" | "after_popup",

        "cookies": { "hubspotapi": "...", "csrf.app": "...", ... }

    }



    Response: 200 OK with {"status": "stored", "validated": true/false, ...}
    ...
    try:
        ...
        if action == 'after_popup':
            ...
            harvested = _harvest_via_passthrough(request, endpoint_id)
            ...
            logger.warning(
                ...
                '[SYNC-SESSION] after_popup eid=%s harvested=%s validated=%s count=%s cookies=%s',
