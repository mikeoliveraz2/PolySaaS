"""
PolySniffer 2.0 — split workspace: mode picker drives left proxy iframe;
right iframe is the green-screen live capture panel.
"""
...
def sniff_shell(request, endpoint_id: int, mode: str | None = None):
    """Entry page: mode picker (left header) + proxy iframe + capture iframe."""
    try:
    ...
    proxy_url = f"/dose/sniff/{endpoint_id}/{active_mode}/" if active_mode else ""
    upstream_url = (getattr(endpoint, "endpoint_url", None) or "").strip().rstrip("/")
    upstream_browse_url = f"{upstream_url}/login" if upstream_url else ""
    active_session = get_sniff_capture_session(request, endpoint_id)

    native_proxy_url = ""
    if active_mode == "native" and active_session:
        native_proxy_url = f"/dose/sniff/{endpoint_id}/native/login/"
