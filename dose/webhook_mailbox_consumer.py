...
def cleanup_expired_mailbox_entries():
    """
    Move past-useful mailbox rows to dead_letter (inspectable).
    Call periodically (e.g. with maintain_webhook_mailboxes or consumer loop).
    """
    try:
        dead_count = WebhookMailbox.expire_old_entries()
        if dead_count > 0:
            logger.info(f"Moved {dead_count} mailbox entries to dead_letter")
        return dead_count
    except Exception as exc:
