...
def _subscriber_facing_messages(selected_apps, *, tenant_name='', stripe_trial_started=True, provisioning_results=None):
    ...
    When synchronous provisioning ran, reflect actual per-app outcomes (not generic 'queued').
    """
    ...
    if labels and provisioning_results:
        ...
        for app_key in selected_apps:
            ...
            if row.get('success'):
                ok_apps.append(label)
            elif row:
                err = (row.get('error') or 'setup failed').strip()
                failed.append(f'{label}: {err}')
        parts = []
        if ok_apps:
            parts.append(f'Ready now: {", ".join(ok_apps)}.')
        if failed:
...
def _subscription_response_payload(subscription, selected_apps, *, tenant_name='', stripe_trial_started=True, user_obj=None, provisioning_results=None):
    ...
    body['provisioning_results'] = provisioning_results or {}
    ...
    if subscription.tenant and 'enable_mattermost' in selected_apps:
        mm_row = (provisioning_results or {}).get('enable_mattermost') or {}
        if mm_row.get('success'):
            try:
                ...
                extra = {}
            body['mm_username'] = extra.get('mm_login_id') or extra.get('mm_username') or extra.get('mattermost_username') or ''
            ...
            body['mm_provisioning_failed'] = True
            body['mm_provisioning_error'] = mm_row.get('error') or 'Mattermost setup did not complete'
    return body
...
class SubscriptionApiViewSet(viewsets.ModelViewSet):
    ...
    def _create_saga(self, request):
        ...
        try:
            ...
            with transaction.atomic():
                ...
                if needs_new_user:
                    import logging
                    logger = logging.getLogger(__name__)
                    logger.warning(f"[DEBUG] Creating user: username={username}, email={email}, password=<redacted len={len(password or '')}>")
        ...
        if tenant_slug:
            provisioning_results = self._register_provisioning_synchronous(
                request, data, tenant_slug, user_obj,
            )
    ...
    def _register_provisioning_synchronous(request, data, tenant_slug, user_obj):
        """Run provisioners inline after transaction commit; return per-app results for API/UI."""
        ...
        try:
            ...
            _mc.close()
            # _mc.close() drops the session's search_path; re-set it on the
            # connection Django will lazily reopen, or migrate silently runs
            # against "public" (already migrated) and no-ops for the tenant.
            set_search_path_for_migrations(tenant.schema_name)
        ...
        for app_key, provisioner in provisioners:
            ...
            # Always create TenantApp before OAuth — OAuth failure must not skip the row
            tapp, _ = TenantApp.objects.get_or_create(
                tenant=tenant,
                app_name=app_name,
                defaults={'status': 'provisioning'},
            )
            kwargs['tenant_app_id'] = tapp.id
            ...
            try:
                ...
                if provision_ok:
                    ...
                    err = result.get('error') or tapp.last_error or 'TenantApp not active after provisioning'
                    logger.warning("[PROVISION] %s returned failure: %s (status=%s)", app_key, err, tapp.status)
            ...
            results[app_key] = {
                'app': app_display,
                'success': provision_ok,
                'error': err if not provision_ok else '',
                'status': tapp.status,
            }
            import time
            time.sleep(5)
                ...
                if username and password:
                    PassthroughCredentialContainer.store(
                        ...
                        credentials={
                            ...
                            'password': password,
                            'email': email,
                            'db': extra.get('odoo_db') or kwargs.get('odoo_db', ''),
                            'api_tokens': {}
                        },
                        ttl_hours=24
                    )
