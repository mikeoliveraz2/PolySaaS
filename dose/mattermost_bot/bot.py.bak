...
PEERS = {
    'github': {
        'aliases': [],
        'mm_username': 'github',
        'bot_token_key': 'BOT_TOKEN_GITHUB',
        'api_key_key': 'ANTHROPIC_API_KEY',
        'router': 'dose.mattermost_bot.routers.github_router',
    },
    'copilot': {
        'aliases': [],
        'mm_username': 'copilot',
        'bot_token_key': 'BOT_TOKEN_COPILOT',
        'api_key_key': 'ANTHROPIC_API_KEY',
        'router': 'dose.mattermost_bot.routers.copilot_router',
    },
    'grok': {
        'aliases': ['supergrok'],
    ...
    'gemini': {
        'aliases': ['gem'],
        'mm_username': 'gemini',
        'bot_token_key': 'BOT_TOKEN_GEMINI',
...
class PolySaaSAIPeersBot:
    ...
    def __init__(self):
        ...
        self._peer_by_user_id: dict = {}
        self._channel_target_cache: dict = {}
    ...
    def start(self):
        ...
        # Python 3.14 may not create a default loop in non-async entrypoints.
        try:
    ...
    def _dispatch(self, post: dict):
        ...
        # In direct-message channels, route plain text to the bot on the other side
        # so users can chat naturally without adding @mentions.
        if not target_peers and channel_id:
            dm_peer = self._infer_direct_message_peer(channel_id, user_id)
            if dm_peer:
                target_peers = [dm_peer]
        ...
        for peer in PEERS.values():
            try:
                ...
                if u and u.get('id'):
                    ...
                    self._peer_by_user_id[u['id']] = _ALIAS_MAP.get(peer['mm_username'], peer['mm_username'])
                    logger.debug("[MM Bot] Registered bot id %s (@%s)", u['id'], peer['mm_username'])
            except Exception:
                pass

    def _infer_direct_message_peer(self, channel_id: str, sender_user_id: str) -> str:
        """Resolve DM channel target peer from the other participant's user id."""
        cached = self._channel_target_cache.get(channel_id)
        if cached is not None:
            return cached

        try:
            channel = self.driver.channels.get_channel(channel_id)
            if (channel or {}).get('type') != 'D':
                self._channel_target_cache[channel_id] = ''
                return ''

            # Mattermost direct channel names are '<user_id_1>__<user_id_2>'.
            channel_name = channel.get('name', '')
            parts = channel_name.split('__') if channel_name else []
            if len(parts) != 2:
                self._channel_target_cache[channel_id] = ''
                return ''

            other_user_id = parts[0] if parts[1] == sender_user_id else parts[1]
            peer_key = self._peer_by_user_id.get(other_user_id, '')
            if peer_key and peer_key in _active_peers():
                self._channel_target_cache[channel_id] = peer_key
                return peer_key
        except Exception as exc:
            logger.debug("[MM Bot] DM peer inference failed for channel %s: %s", channel_id, exc)
