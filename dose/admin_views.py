...
LIGHT_THEMES = sorted([
    "cerulean", "cosmo", "flatly", "journal", "litera", "lumen", "lux", "materia",
    "minty", "pulse", "sandstone", "simplex", "sketchy", "spacelab", "united", "yeti",
])
...
def select_theme_api(request):
    ...
    return JsonResponse({
        'light_theme': profile.light_theme if profile else LIGHT_THEME_DEFAULT,
        'dark_theme': profile.dark_theme if profile else DARK_THEME_DEFAULT,
...
def pt_admin_generic_passthrough_view(request, endpoint, subpath=None):
    ...
    logger.info(
        "[PASSTHROUGH VIEW CSRF_EXEMPT] endpoint=%s, subpath=%s, path=%s, method=%s",
        endpoint, subpath, request.path, request.method,
    )
