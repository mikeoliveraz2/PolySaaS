...
def pt_admin_generic_passthrough_view(request, trigger, subpath=None):
    ...
    print(f"[PASSTHROUGH VIEW] trigger={trigger}, subpath={subpath}, path={request.path}")

    from dose.utils import get_current_tenant
    from dose.models import UserTenantMembership
    from dose.passthrough.registry import get_handler          # ← Single registry
    ...
    print(f"[VIEW] Using generic forwarder for {trigger}")
    return forward_request_standardized(request, subpath, handler=handler)


@never_cache
@login_required
def passthrough_embed_view(request, trigger):
    print(f"[EMBED VIEW] trigger={trigger}")
    # Add your embed logic here if needed, using the same registry import
    return HttpResponse(f"Embed view for {trigger} - not fully implemented yet")


@never_cache
@login_required
def passthrough_display_shell_view(request):
    """Display shell view for passthrough content - provides a container page
    that can load and display passthrough content from external services."""
    from dose.utils import get_current_tenant
    ...
    # Get trigger and optional endpoint_id from query parameters
    ...
    # If we have a trigger, try to get endpoint info
    if trigger:
        handler = get_handler(trigger)
        context['handler_name'] = handler.__class__.__name__ if handler else None
    ...
    return render(request, 'admin/passthrough_display_shell.html', context)
