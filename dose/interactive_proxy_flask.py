#!/usr/bin/env python3
"""Flask-based interactive proxy to preview and click through remote pages."""
...
def doseify_html(html: str) -> str:
    ...
    # Now fix relative and root-relative paths in href/src attributes
    # Convert /scp/path to /admin/osticket/path (strip the /scp/ prefix)
    import re
...
def index():
    ...
    <body style='font-family:Arial,sans-serif; padding:30px;'>
      <h2>Interactive Proxy</h2>
      <p>Click the button below to load OSTicket through the proxy.</p>

      <div style="margin-bottom:12px;">
        <button id="load_btn" type="button" style="font-size:16px;padding:10px 18px;background:#007bff;color:#fff;border:none;border-radius:6px;" onclick="window.location='/admin/osticket/login.php';">Load OSTicket</button>
      </div>
    </body>
    </html>
    """
...
def osticket_proxy(path):
    ...
    # Strip /scp/ if it was included in the path (in case HTML has /scp/something)
    if path.startswith("scp/"):
        path = path[4:]  # Remove "scp/" prefix

    target = REAL_BASE + path
    ...
    try:
        if request.method == "POST":
            resp = sess.post(target, data=request.form, timeout=15, allow_redirects=True)
        else:
