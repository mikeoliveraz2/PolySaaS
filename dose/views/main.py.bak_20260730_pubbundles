...
def pt_dose_generic_passthrough_view(request, trigger, subpath=None):
    ...
    trigger_host = (trigger or '').strip('/').lower()
    allowed_hosts = {
        (service.get('trigger') or '').lower()
        for service in context.get('passthrough_services', [])
    }
    if trigger_host not in allowed_hosts:
        raise Http404('Not found')

    endpoint = _resolve_landing_passthrough_endpoint(trigger_host)
    if endpoint is None:
    ...
    embed_title = endpoint.menu_title or trigger_host.replace('.', ' ').replace('-', ' ').title()
    embed_head = ''
    embed_body = ''
    if raw_html and raw_html.startswith('REDIRECT:'):
        ...
        processed = _retarget_passthrough_prefixes_for_dose(processed, trigger_host)
        head_inner, body_html = _split_html_document_for_jazzmin_embed(processed)
        embed_head = mark_safe(head_inner)
        embed_body = mark_safe(
            f'<div class="polysaas-passthrough-scope" data-polysaas-embed-trigger="{trigger_host}">{body_html or ""}</div>'
        )
    else:
        embed_body = mark_safe(
            '<div class="landing-passthrough-message error">'
    ...
    context.update({
        ...
        'passthrough_embed_trigger': trigger_host,
