...
# to check whether an app name was a substring of the endpoint hostname (fails for
# localhost:PORT everywhere in local dev). Replaced with an explicit PassThroughEndpoint.slug
# == TenantApp.app_name match, same convention already used in context_processors.py.
# BINGO: Dose Home Passthrough Visibility Fix — 2026-08-02
...
class UnreadDoseMessagesView(APIView):
    ...
    def _unread_queryset(self, request):
        from dose.models import DoseMessage

        qs = DoseMessage.objects.filter(is_read=False).order_by('-created_at')
        if not request.user.is_superuser:
            qs = qs.filter(user=request.user)
        return qs

    @staticmethod
    def _serialize_messages(queryset):
        return [
            {
                'id': msg.id,
                'message': msg.message,
    ...
    def get(self, request, *args, **kwargs):
        ...
        messages = self._serialize_messages(self._unread_queryset(request))
        return JsonResponse({'count': len(messages), 'messages': messages})

    def post(self, request, *args, **kwargs):
        from django.http import JsonResponse

        updated = self._unread_queryset(request).update(is_read=True)
        return JsonResponse({
            'count': 0,
            'messages': [],
            'updated': updated,
    ...
    def patch(self, request, *args, **kwargs):
        ...
        filt = {'id': message_id, 'is_read': False}
        if not request.user.is_superuser:
            filt['user'] = request.user
        updated = DoseMessage.objects.filter(**filt).update(is_read=True)
        count = self._unread_queryset(request).count()
        return JsonResponse({'success': updated > 0, 'updated': updated, 'count': count})
...
def _build_landing_page_context(request):
    ...
    try:
        ...
        if current_tenant:
            ...
            with tenant_schema_search_path(current_tenant):
                _subscribed = set(
    ...
    def _endpoint_visible(endpoint):
        # FIX 2026-08-02 (owner-approved): the endpoint is the endpoint — visibility is
        # decided by matching endpoint.slug against this tenant's own TenantApp rows, not
        # by guessing whether the app name happens to appear as a substring of the
        # hostname (that failed for every locally-hosted app, e.g. "odoo" is not a
        # substring of "localhost:8086"). Gmail has no APP_CHOICES/TenantApp entry, so it
        # stays always-visible like before.
        return endpoint.slug == 'gmail' or endpoint.slug in _subscribed
    ...
    for endpoint in passthrough_endpoints:
        ...
        host = urlparse(endpoint.endpoint_url or "").netloc.lower()
...
def _resolve_landing_passthrough_endpoint(endpoint_host):
    """Resolve the tenant PassThroughEndpoint by exact endpoint_url host."""
    from dose.models import PassThroughEndpoint
    from urllib.parse import urlparse

    host_key = (endpoint_host or '').strip().lower()
    if not host_key:
        return None
    for candidate in PassThroughEndpoint.objects.filter(is_enabled=True).order_by('-id'):
        host = urlparse(candidate.endpoint_url or '').netloc.lower()
        if host == host_key:
            return candidate
    return None
...
def _retarget_passthrough_prefixes_for_dose(html, trigger):
    """Rewrite /pt/admin/… links in embedded HTML to /pt/dose/… for the landing shell."""
    if not html:
        return html
    ...
    return html.replace(admin_prefix, dose_prefix)
def _process_upstream_html_for_embed(handler, raw_html, request, endpoint_url):
    """Run handler HTML rewrite for dose-home embed (no admin Jazzmin wrap)."""
    if not raw_html:
        return raw_html
    if handler and hasattr(handler, 'process_html_response'):
        processed = handler.process_html_response(
            raw_html, request, endpoint_url=endpoint_url,
        )
        if isinstance(processed, tuple):
            return processed[0] if processed[0] else raw_html
        from django.http import HttpResponse
        if isinstance(processed, HttpResponse):
            try:
                return processed.content.decode('utf-8', errors='replace')
            except Exception:
                return raw_html
...
def _split_html_document_for_dose_embed(html):
    """Split upstream HTML into head/body fragments for landing_page embed slots."""
    from dose.passthrough.forwarding import _extract_head_and_body
    return _extract_head_and_body(html or '')
...
def pt_dose_generic_passthrough_view(request, endpoint=None, subpath=None):
    ...
    allowed = {
        (service.get('endpoint_host') or '').lower()
        for service in context.get('passthrough_services', [])
    }
    if endpoint_host not in allowed:
        raise Http404('Not found')

    endpoint = _resolve_landing_passthrough_endpoint(endpoint_host)
    if endpoint is None:
    ...
    embed_title = endpoint.menu_title or endpoint_host.replace('.', ' ').replace('-', ' ').title()
    embed_head = ''
    embed_body = ''
    if raw_html and raw_html.startswith('REDIRECT:'):
        ...
        processed = _retarget_passthrough_prefixes_for_dose(processed, endpoint_host)
        head_inner, body_html = _split_html_document_for_dose_embed(processed)
        embed_head = mark_safe(head_inner)
        embed_body = mark_safe(
            f'<div class="polysaas-passthrough-scope" data-polysaas-embed-host="{endpoint_host}">{body_html or ""}</div>'
        )
    else:
        embed_body = mark_safe(
            '<div class="landing-passthrough-message error">'
    ...
    context.update({
        ...
        'passthrough_embed_trigger': slug_key,
