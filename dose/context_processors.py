...
def admin_navigation(request):
    ...
    try:
        ...
        # Track both titles and trigger_paths to prevent duplicates
        seen_titles = set()
        seen_trigger_paths = set()

        for endpoint in endpoints:
            print(f"[ADMIN_NAV] Endpoint: {endpoint.trigger_path} - {endpoint.menu_title}")

            # Check for duplicates by trigger_path (more reliable than title)
            trigger_path_normalized = endpoint.trigger_path.strip('/').lower()
            if trigger_path_normalized in seen_trigger_paths:
                print(f"[ADMIN_NAV] Skipping duplicate trigger_path: {endpoint.trigger_path}")
                continue
            seen_trigger_paths.add(trigger_path_normalized)

            # Also check by title as backup
            title = endpoint.menu_title or endpoint.trigger_path.title()
            if title in seen_titles:
                print(f"[ADMIN_NAV] Skipping duplicate title: {title}")
                continue
            seen_titles.add(title)

            # Route to appropriate view using standardized /pt/[context]/[trigger] format
            ...
            print(f"[ADMIN_NAV] Using standardized passthrough URL: {url} (context: {context}, trigger: {endpoint.trigger_path})")
