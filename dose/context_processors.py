...
def admin_navigation(request):
    ...
    try:
        ...
        for endpoint in endpoints:
            ...
            # READINESS 2026-08-01 (owner-approved, frozen-file exception): a PassThroughEndpoint
            # row is created (provisioner Step 1) before the underlying app account/database is
            # actually finished provisioning (Step 2+), so it can appear here before it's usable.
            # TenantApp.status is the existing source of truth for that ('provisioning'/'active'/
            # 'error'/'disabled' — see dose/models/tenant_app.py). If no TenantApp row matches this
            # endpoint's slug (e.g. Gmail, which isn't an APP_CHOICES entry), default to ready=True
            # so nothing that previously worked without app-registry tracking gets hidden/disabled.
            ready = True
            app_status = 'active'
            try:
                if tenant and tenant.schema_name:
                    from django.db import connection as _ta_connection
                    with _ta_connection.cursor() as _ta_cursor:
                        _ta_cursor.execute(f'SET search_path TO "{tenant.schema_name}", public')
                    from dose.models import TenantApp
                    _tenant_app = TenantApp.objects.filter(app_name=endpoint.slug).first()
                    if _tenant_app:
                        app_status = _tenant_app.status
                        ready = (app_status == 'active')
