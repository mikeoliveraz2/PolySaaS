...
class InputDispatchTests(SimpleTestCase):
    ...
    def test_scrolling_uses_the_page_wheel(self):
        page = MagicMock()
...
class HostFallbackLookupTests(SimpleTestCase):
    def setUp(self):
        self.key = ("olient", "app.slack.com")
        self.run = _NativeBrowserRun()
        with _RUNS_LOCK:
    ...
    def test_input_finds_the_run_when_schema_casing_differs(self):
        queued = queue_native_input(
            tenant_schema="Olient",
            endpoint_host="APP.SLACK.COM",
    ...
    def test_input_finds_the_only_host_run_when_schema_is_wrong(self):
        queued = queue_native_input(
            tenant_schema="wrong-schema",
            endpoint_host="app.slack.com",
            event={"kind": "text", "text": "x"},
        )
