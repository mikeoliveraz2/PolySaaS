...
class SlackNativeSniffTests(SimpleTestCase):
    ...
    def test_slack_wireframe_has_contact_and_sale_actions(self):
        ...
        self.assertIn("New contact", html)
        self.assertIn("New sale", html)
        self.assertIn('name="name"', html)
        self.assertIn('name="partner_name"', html)
        self.assertIn('name="order_reference"', html)
    ...
    def test_production_passthrough_offers_supported_browser_url_copy(self):
        ...
        self.assertIn("Open full screen in new tab", template)
        self.assertIn("polysaas-passthrough-fullscreen", template)
        self.assertIn("ps_fullscreen=1", template)
        self.assertIn("lockPassthroughLayout", template)
        self.assertIn("translateZ(0)", template)
        self.assertIn("polysaas-slack-wireframe-embed", template)
        self.assertIn("polysaas-odoo-embed", template)
        self.assertIn("content-column default for all passthrough", template)
        self.assertNotIn("psForceHideSidebar", template)
        sidebar = (
            Path(__file__).parents[1]
            / "templates"
            / "admin"
        ...
        self.assertIn("pss-pt-fullscreen", sidebar)
        self.assertIn("service.fullscreen_url", sidebar)
    ...
    def test_mailbox_poll_includes_wireframe_native_and_passthrough(self):
        self.assertTrue(
            _include_mailbox_in_poll(mode="passthrough", wireframe=True, source="slack")
        )
        self.assertTrue(
            _include_mailbox_in_poll(mode="native", wireframe=True, source="slack")
        )
        self.assertTrue(
            _include_mailbox_in_poll(mode="passthrough", wireframe=False, source="slack")
        )
        self.assertFalse(
            _include_mailbox_in_poll(mode="native", wireframe=False, source="slack")
        )
        self.assertFalse(
            _include_mailbox_in_poll(mode="passthrough", wireframe=True, source="")
        )

    def test_mailbox_outcome_from_atomic_results(self):
        self.assertEqual(
            _mailbox_outcome(
                {"status": "processed", "results": [{"status": "success", "partner_id": 11}]}
            ),
            "partner #11",
        )
        self.assertEqual(
            _mailbox_outcome(
                {
                    "status": "processed",
                    "results": [{"status": "success", "order_name": "S00002"}],
                }
            ),
            "S00002",
        )
    ...
    def test_wireframe_mailbox_payload_fields_are_kept(self):
        row = SimpleNamespace(
            id=2,
            source="slack",
            action_path="/events/slack/webhook/contact",
            status="processed",
            result={
                "status": "processed",
                "matched": 1,
                "executed": 1,
                "results": [{"status": "success", "partner_id": 12}],
            },
            error="",
            created_at=None,
            processed_at=None,
