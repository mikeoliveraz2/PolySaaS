...
class SlackNativeSniffTests(SimpleTestCase):
    ...
    def test_start_native_session_launches_direct_slack_browser(
        ...
        kwargs = start_browser.call_args.kwargs
        self.assertEqual(kwargs["endpoint_url"], "https://polysaasworkspace.slack.com")
        self.assertEqual(kwargs["tenant_schema"], "polysaasonline")
        self.assertEqual(kwargs["capture_session_id"], 44)
    ...
    def test_slack_handler_remembers_only_slack_cookies(self):
        ...
        with tempfile.TemporaryDirectory() as media_root:
            with override_settings(MEDIA_ROOT=media_root):
                handler.remember_native_session_cookies(
                    [
                        {"name": "d", "value": "secret", "domain": ".slack.com", "path": "/"},
                        {"name": "tracker", "value": "x", "domain": ".ads.example", "path": "/"},
                    ],
                    tenant_schema="olient",
                    endpoint_host="app.slack.com",
                )
                cookies = handler.native_session_cookies(
                    tenant_schema="olient", endpoint_host="app.slack.com"
                )
                store = (
                    Path(media_root)
                    / "polysniffer"
                ...
                self.assertEqual(cookies[0]["name"], "d")
